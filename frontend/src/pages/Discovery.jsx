...
export default function Discovery() {
  ...
  const scan = async (event) => {
    ...
    try {
      ...
      setResults(response.data.map((item) => ({ ...item, name: item.hostname || `Device-${item.ip_address.replaceAll('.', '-')}`, location: 'Network discovery', group_name: '' })));
      setMessage({ type: 'success', text: `${response.data.length} active device(s) found.` });
  ...
  const importSelected = async () => {
    ...
    try {
      ...
      setMessage({ type: 'success', text: `${response.data.imported.length} device(s) imported; ${response.data.skipped.length} skipped.` });
      setSelected(new Set());
    } catch (error) { setMessage({ type: 'error', text: getApiErrorMessage(error) }); }
    finally { setLoading(false); }
  };
  ...
  return <div>
    <div className="page-title"><div><h2>Network Discovery</h2><p>Find assets through ICMP and TCP ports before adding them to monitoring.</p></div></div>
    <form className="glass-card platform-form" onSubmit={scan}>
      <div className="form-group grow"><label className="form-label">Network, range, or IP</label><input className="form-input" value={target} onChange={e => setTarget(e.target.value)} required /></div>
      <div className="form-group grow"><label className="form-label">Ports TCP</label><input className="form-input" value={ports} onChange={e => setPorts(e.target.value)} /></div>
      ...
      {methods.includes('SNMP') && <><div className="form-group"><label className="form-label">SNMP Version</label><select className="form-select" value={snmpVersion} onChange={e=>setSnmpVersion(e.target.value)}><option value="2c">SNMPv2c</option><option value="3">SNMPv3</option></select></div>{snmpVersion==='2c'?<div className="form-group"><label className="form-label">Community (will not be stored)</label><input className="form-input" type="password" value={snmpCommunity} onChange={e => setSnmpCommunity(e.target.value)} /></div>:<><div className="form-group"><label className="form-label">SNMPv3 User</label><input className="form-input" value={snmpV3.username} onChange={e=>setSnmpV3({...snmpV3,username:e.target.value})}/></div><div className="form-group"><label className="form-label">Auth key</label><input className="form-input" type="password" value={snmpV3.auth_key} onChange={e=>setSnmpV3({...snmpV3,auth_key:e.target.value})}/></div><div className="form-group"><label className="form-label">Privacy key</label><input className="form-input" type="password" value={snmpV3.priv_key} onChange={e=>setSnmpV3({...snmpV3,priv_key:e.target.value})}/></div></>}</>}
      <button className="btn btn-primary" disabled={loading || !methods.length}><Radar size={17} /> {loading ? 'Scanning…' : 'Start Discovery'}</button>
    </form>
    {message && <div className={`notice ${message.type}`}>{message.text}</div>}
    <div className="glass-card" style={{ overflow: 'auto' }}>
      {!results.length ? <div className="empty-platform"><Search size={40} /><strong>No discovery performed</strong><span>Enter a target and start a controlled scan.</span></div> : <>
        <div className="table-toolbar"><span>{selected.size} selected</span><button className="btn btn-primary" disabled={!selected.size || loading} onClick={importSelected}><Download size={16} /> Add Selected</button></div>
        <table className="custom-table"><thead><tr><th></th><th>IP / Status</th><th>Editable Name</th><th>Type</th><th>Location / Group</th><th>Latency</th><th>Ports</th></tr></thead><tbody>
          {results.map((item, index) => <tr key={item.ip_address}>
            ...
            <td><input className="form-input compact" value={item.location} onChange={e => update(index, 'location', e.target.value)} /><input className="form-input compact" placeholder="Group" value={item.group_name} onChange={e => update(index, 'group_name', e.target.value)} /></td>
