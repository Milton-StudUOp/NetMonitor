...
export default function MetricsMonitoring(){
  const navigate=useNavigate(); const [rows,setRows]=useState([]); const [search,setSearch]=useState(''); const [selectedDeviceId,setSelectedDeviceId]=useState(''); const [loading,setLoading]=useState(true); const [error,setError]=useState('');
  ...
  return <div className="data-page">
    <div className="page-title"><div><h2>Metrics Monitoring</h2><p>Windows and Linux system, storage, network, and process telemetry.</p></div><div className="row-actions"><button className="btn btn-secondary" onClick={load}><RefreshCw size={15} className={loading?'spin':''}/>Refresh</button><button className="btn btn-primary" onClick={()=>navigate('/metrics-discovery')}>Discover Metrics</button></div></div>
    ...
    <section className="glass-card analytics-panel">
      <div className="panel-title"><div><h3>Monitored devices</h3><p>Latest normalized Windows and Linux metrics by device.</p></div></div>
